...
import { TrendingUp, Users, Calendar, Scissors, CheckCircle, XCircle, Clock, Shield } from 'lucide-react';

const AdminDashboard = () => {
  const [stats, setStats] = useState<any>(null);
  const [appointments, setAppointments] = useState<any[]>([]);
  const [users, setUsers] = useState<any[]>([]);
  const [tab, setActiveTab] = useState<'bi' | 'users'>('bi');
        ...
        <div>
          ...
          <div className="flex gap-4 mt-4">
            <button 
              ...
              className={`px-6 py-2 rounded-full font-bold text-sm transition-all ${tab === 'bi' ? 'bg-brand-dark text-white' : 'bg-white text-brand-dark border border-brand-gold/20'}`}
            >
              Dashboard BI
            </button>
            <button 
              onClick={() => setActiveTab('users')}
              className={`px-6 py-2 rounded-full font-bold text-sm transition-all ${tab === 'users' ? 'bg-brand-dark text-white' : 'bg-white text-brand-dark border border-brand-gold/20'}`}
            >
              Gestão de Usuários
            </button>
          </div>
            ...
            <table className="w-full text-left">
              <thead className="bg-brand-light text-brand-dark text-xs uppercase font-black tracking-widest">
                <tr>
                  ...
                  <th className="px-10 py-5">Perfil</th>
              ...
              <tbody className="divide-y divide-gray-100 font-sans">
                {users.map((u: any) => (
                  <tr key={u.id} className="hover:bg-brand-light/20 transition-colors">
                    <td className="px-10 py-6 font-bold text-brand-dark">{u.nome}</td>
                    <td className="px-10 py-6 text-gray-600">{u.email}</td>
                    <td className="px-10 py-6">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-black tracking-widest ${u.role === 'ADMIN' ? 'bg-brand-dark text-white' : 'bg-brand-gold text-brand-dark'}`}>
                        {u.role}
                      </span>
                    </td>
                    <td className="px-10 py-6">
                      <span className={`flex items-center gap-2 text-xs font-bold ${u.ativo ? 'text-green-600' : 'text-red-600'}`}>
                        <div className={`w-2 h-2 rounded-full ${u.ativo ? 'bg-green-600' : 'bg-red-600'}`} />
                    ...
                    <td className="px-10 py-6">
                      <button 
                        ...
                        className={`px-4 py-1.5 rounded-full text-[10px] font-black tracking-widest transition-all ${u.ativo ? 'bg-red-50 text-red-600 hover:bg-red-600 hover:text-white' : 'bg-green-50 text-green-600 hover:bg-green-600 hover:text-white'}`}
                      >
