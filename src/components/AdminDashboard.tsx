...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchSiteSettings = async () => {
      ...
      try {
        const textRes = await axios.get(`/api/settings/promo_text?t=${Date.now()}`);
        setPromoText(textRes.data.value || '');
        const activeRes = await axios.get(`/api/settings/promo_active?t=${Date.now()}`);
        setPromoActive(activeRes.data.value === 'true' || activeRes.data.value === true);
        const styleRes = await axios.get(`/api/settings/promo_style?t=${Date.now()}`);
        if (styleRes.data.value) setPromoStyle(JSON.parse(styleRes.data.value));
      } catch { /* ignore */ }
    };
              ...
              <div className="bg-white rounded-3xl shadow-2xl border border-brand-gold/10 overflow-hidden">
                <div className="overflow-x-auto">
                  <div className="min-w-[800px]">
                    ...
                    {/* Grid Body */}
                    <div className="h-[700px] overflow-y-auto p-0.5 relative bg-brand-light/5">
                      {HALF_HOURS.map((hora) => (
